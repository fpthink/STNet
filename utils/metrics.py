...
def fromBoxToPoly_waymo(box):
    return Polygon(tuple(box.corners()[[0, 1]].T[[0, 1, 5, 4]]))


def estimateOverlap(box_a, box_b, dim=2, dataset_type='kitti'):
    if dataset_type.upper() == 'WAYMO':
        Poly_anno = fromBoxToPoly_waymo(box_a)
        Poly_subm = fromBoxToPoly_waymo(box_b)
    else:
